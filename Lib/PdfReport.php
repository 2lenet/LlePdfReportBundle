...
class PdfReport extends \TCPDF {
    ...
    public function __construct($xml_report_string) {
        ...
        if((intval($this->rdata['pageWidth'])) > (intval($this->rdata['pageHeight']))) $orientation = 'L'; else $orientation = 'P'; 
	...
	$previousDataObj=Null;
        ...
        foreach ($datacoll as $dataObj) {
            ...
            foreach($this->rdata->group as $group) {
			    ...
			    $this->dataObj = $dataObj;
                            if($startPage) $this->newPage();
                        }

                        $this->generateGroupItem('groupHeader', $group->groupHeader);
                        $current_group[$k] = $datag;
                        for($i = $k+1;$i < count($current_group);$i++) $current_group[$i] = null;
                    }
                }
                $k++;
            }
		    ...
		    $maxY = $this->getPageHeight() 
			    - $this->bottomMargin 
			    - $this->rdata->summary->band['height']
    ...
    public function generateGroupItem($key, $group=null, $maxY=0) {
	    ...
	    // version optimiste.
            ...
            foreach ($group->band->children() as $node) {
                ...
                if (($group->band['splitType']=='Stretch') && ($this->getY() > $this->maxY) ) $this->maxY = $this->getY();
            }
            if ($group->band['splitType']=='Stretch') {
                $this->setY($this->maxY);
                if ($this->getY() < ($this->group_y + $group->band['height'])) $this->setY($this->group_y + $group->band['height']);
            } else {
    ...
    public function generateTextField($item) {
        if($item->textFieldExpression == '$V{PAGE_NUMBER}'){
           ...
           $data = $this->getFieldData((string) $item->textFieldExpression, $this->dataObj, $item['pattern']); 
        }
    ...
    public function getAlign($item) {
        if($item->textElement) {
            if($item->textElement['textAlignment']=='Center') return 'C';
            if($item->textElement['textAlignment']=='Right') return 'R';
        }
        return 'L';
    }
    ...
    public function generateRectangle($item) {
        ...
        $this->Rect(
            $item->reportElement['x'] + $this->decX + $this->lMargin, 
            $y, 
            $item->reportElement['width'], 
            $item->reportElement['height'],
            $style,
            array(),
            $this->hex2rgb($item->reportElement['backcolor']));
    }
    ...
    public function generateLine($item) {
        ...
        if( $item->reportElement['stretchType'] == "RelativeToBandHeight") {
            $height = $this->maxY - $y;
            if ($height < $item->reportElement['height']) $height = $item->reportElement['height'];
        } else {
            $height = $item->reportElement['height'];
        }
